...
class WhatsAppEventBot {
  ...
  cleanOldFlyers() {
    try {
      const maxAgeMs = this.maxImageAgeDays * 24 * 60 * 60 * 1000;
      ...
      files.forEach((file) => {
        ...
        try {
          ...
          if (stats.mtimeMs < cutoffTime) {
            ...
            deletedCount++;
            console.log(`🗑️  Deleted old flyer: ${file}`);
          }
        } catch (err) {
      ...
      if (deletedCount > 0) {
        ...
        console.log(
          `✅ No old files to clean (keeping files < ${this.maxImageAgeDays} days old)`
        );
      }
    } catch (error) {
  ...
  scheduleCleanup() {
    const scheduleNextCleanup = () => {
      ...
      const msUntilCleanup = next3AM.getTime() - now.getTime();

      console.log(`🕐 Next cleanup scheduled for: ${next3AM.toLocaleString()}`);
